...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center_row = (goal.row + start.row) / 2
            x_center_col = (goal.col + start.col) / 2
            x_center = Node(x_center_row, x_center_col)

            # Compute rotation matrix from ellipse to world frame - C

            dis_a = np.array([[(goal.row - start.row) / c_min], [(goal.col - start.col) / c_min], [0]])
            eye = np.array([[1], [0], [0]])
            dot_M = np.dot(dis_a, eye.T)
            U, S, V_Trans = np.linalg.svd(dot_M, full_matrices=True)
            det_prod = np.linalg.det(U) * np.linalg.det(V_Trans.T)

            diag_M = np.diag([1, 1, det_prod])
            # print('daig_M', diag_M)
            dot_M_VTrans = np.dot(diag_M, V_Trans)
            C = np.dot(U, dot_M_VTrans)

            # Compute diagonal matrix - L

            L = np.diag([c_best / 2, np.sqrt(c_best ** 2 - c_min ** 2) / 2, np.sqrt(c_best ** 2 - c_min ** 2) / 2])

            # Cast a sample from a unit ball - x_ball
            nm = 1
            dm = 2
            v = np.random.normal(0, 1, (nm, dm + 2)) / np.linalg.norm(np.random.normal(0, 1, (nm, dm + 2)), axis=-1, keepdims=True)
            # Map ball sample to the ellipsoid - x_rand
            x_ball = np.zeros((3, 1))
            arr = np.atleast_2d(v[0, :dm]).T

            x_ball[0:2, :] = arr

            # Map ball sample to the ellipsoid - x_rand
            dot_L_x_ball = np.dot(L, x_ball)
            arr_x_center = np.array([x_center.row, x_center.col, 0])
            x = np.dot(C, dot_L_x_ball) + np.atleast_2d(arr_x_center).T
            point = [x[0][0], x[1][0]]
