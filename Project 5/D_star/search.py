...
class DStar:
    ...
    def get_neighbors(self, node):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                # Check range
                if row + i < 0 or row + i >= len(self.grid) or col + j < 0 or col + j >= len(self.grid[0]):
                    continue
    ...
    def process_state(self):
        ...
        if nd is not None:
         k = self.get_k_min()
         neighbors = self.get_neighbors(nd)
         self.delete(nd)
        ...
        if k < nd.h:
            for n in neighbors:
                neighbor_path_goal= n.h + self.cost(n, nd)
                if (n.h <= k) and (nd.h > neighbor_path_goal):
                    nd.parent = n
                    nd.h = n.h + self.cost(n, nd)

        if k == nd.h:
            for n in neighbors:
                node_path_goal = nd.h + self.cost(n, nd)
                if ((n.parent == nd) and (n.h != node_path_goal)) or \
                        ((n.tag == "NEW") or \
                         ((n.parent != nd) and (n.h > node_path_goal))):
                    n.parent = nd
                    self.insert(n, nd.h + self.cost(nd, n))
          ...
          for n in neighbors:
            ...
            if (((n.parent == nd) and (n.h != node_path_goal)) or \
                ...
                if (n.parent != nd) and (n.h > node_path_goal):
                        self.insert(nd, nd.h)
                else:
                    if (n.parent != nd) and (nd.h > neighbor_path_goal) and \
                                (n.tag == "CLOSED") and \
                                (n.h > k):
                            self.insert(n, n.h)
    ...
    def modify_cost(self, obstacle_node, neighbor):
        ...
        #print(obstacle_node.row, obstacle_node.col)
        obst = [obstacle_node, obstacle_node.h]
        if (obst[0].tag == "CLOSED"):
            self.insert(obst[0], obst[1])
        #### TODO END ####
        ...
        for n in neighbors:
            ...
            if (n.is_obs == False):
                ...
                # using self.modify_cost
                  ...
                  if n.tag == "CLOSED":
                    infinity = math.inf
                    self.insert(n, infinity)

                  for hosa_neighbor in neighbors:

                    self.modify_cost(nd, hosa_neighbor)
    ...
    def run(self):
        ...
        self.goal.h = 0
        goal = [self.goal, self.goal.h]
        self.insert(goal[0], goal[1])
        start = [self.start, self.start.h]
        node_present = start[0]

        while True:
            k = self.process_state()
            if k == -1 or (start[0].tag != "NEW" and start[0].tag != "OPEN"):
                break

        # Visualize the first path if found
        self.get_backpointer_list(start[0])
        ...
        while node_present != goal[0]:
            ...
            parent_nd = node_present.parent
            node_present = parent_nd
